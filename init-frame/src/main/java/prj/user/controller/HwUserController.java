...
@Controller
@RequestMapping(value = "/prj_user/hwuser")
public class HwUserController extends BaseController
{
	private static final UUID UUID_GENERATOR = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB");
	private static final Logger logger = Logger.getLogger(HwUserController.class);
	@Autowired  
    private HwUserDao hwUserDao;  	
    ...
    public Map<String, Object> insert(@Valid @RequestBody HwUser hwUser,Errors errors)
	{
		if(errors.hasErrors())
			return error(errors);	
         ...
         hwUser.setToken(getAutoValue(java.lang.String.class));
		
    	try{
    		hwUserDao.insert(hwUser);
			return ok("操作成功",hwUser);
    ...
    public Map<String, Object> update(@Valid @RequestBody HwUser hwUser,Errors errors)
	{
		if(errors.hasErrors())
			return error(errors);	
    	...
    	try{
    	   for(String s : strArr){
    		   if (StringUtils.hasText(s)) {
				  Object[] params = HwUser.keyString2Array(s);
				  hwUserDao.delete((String)params[0]);
    		   }
    	   }
    ...
    public Map<String, Object> delete(@PathVariable("token") String token) 
	{
    	try{
		   if (StringUtils.hasText(token)) {
			  Object[] params = HwUser.keyString2Array(token);
			  hwUserDao.delete((String)params[0]);
		   }
			return ok(getMessage("msg.operation.success"));
    	}catch(DataAccessException ex){
    ...
    public Map<String,Object> detail(@PathVariable("token") String token) {    	
   		HwUser hwUser = hwUserDao.getObjFully(token);
   		return ok(hwUser);
    }
    ...
    public  Map<String,Object> queryList() {   	
        List<Object> list=toList(hwUserDao.queryListFully());
        return ok(list);
    }
    @ResponseBody
